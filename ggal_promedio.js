...
function calcPromedio(){
  ...
  strikeKeys.forEach(strike=>{
    ...
    sec.innerHTML=`
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid var(--border2)">
        ...
        <span style="font-family:var(--mono);font-size:11px;color:var(--muted)">Balance: ${netLotes>=0?'+':''}${netLotes} lotes</span>
      </div>
  ...
  if(strikeKeys.length>1){
    ...
    grandEl.style.cssText='grid-column:1/-1;background:var(--surface);border:1px solid rgba(232,184,75,0.3);border-radius:8px;padding:12px 16px;display:flex;align-items:center;gap:20px;font-family:var(--mono)';
    grandEl.innerHTML=`
      <span style="font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.6px">Total general</span>
      <span style="font-size:9px;color:var(--muted)">${strikeKeys.length} strikes</span>
      <span style="margin-left:auto;font-size:16px;font-weight:700;color:${gc}">${fmtN(grandNet)}</span>`;
